...
public class LexicalizedParser extends ParserGrammar implements Serializable  {
  ...
  //This is code where tree is parsed.
  public Tree parse(List<? extends HasWord> lst) {
    try {
    	System.out.println("lp.apply code is executed here");
    	ParserQuery pq = parserQuery();
    	if (pq.parse(lst)) {  // If parse gets  true, 
	        Tree bestparse = pq.getBestParse(); // get Best parse
	      //  List<ScoredObject<Tree>> bestparses = pq.getKBestParses(5); //Can get top k parse result
	        final PrintWriter pw = new PrintWriter(System.out, true);   //writer function for printing tree 
	        TreePrint tp = new TreePrint("penn,typedDependencies"); // penn -> seg tree 
	       // tp.printTrees(bestparses,"", pw); // print all k trees.  
	   //     System.out.println(bestparses);
	        
	        // -10000 denotes unknown words
	        bestparse.setScore(pq.getPCFGScore() % -10000.0);
	        System.out.println(bestparse.score());
  ...
  public List<ScoredObject<Tree>> kparse (List<? extends HasWord> lst) {
	    try {
	    	System.out.println("for k parses");
	    	ParserQuery pq = parserQuery();
	    	if (pq.parse(lst)) {  // If parse gets  true, 
		        List<ScoredObject<Tree>> bestparses = pq.getKBestParses(5); //Can get top k parse result
		        final PrintWriter pw = new PrintWriter(System.out, true);   //writer function for printing tree 
